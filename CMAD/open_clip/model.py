...
from prototype_utils import Prototype_global, Prototype_patch
...
class CMAD(nn.Module):
    def __init__(
            ...
            cast_dtype: Optional[torch.dtype] = None,
            output_dict: bool = False,
            imagebind: Optional[Any] = None,
    ):
        super().__init__()
        self.output_dict = output_dict
        
        self.prototype_global = Prototype_global()
        self.prototype_patch = Prototype_patch()
    ...
    def forward(self, tokenizer, data_dict: Optional[torch.Tensor] = None, text: Optional[torch.Tensor] = None, few_shot_samples = None, cfg = None):
        token, Fp_list = data_dict['token'].cuda(non_blocking=True), data_dict['Fp_list'].cuda(non_blocking=True) # b c | b m c h w
        Fp_list = rearrange(Fp_list, 'b m c h w -> b m (h w) c')
        ...
        if few_shot_samples is not None:
            ...
            token_n = few_shot_samples['global'].cuda(non_blocking=True) # s c
            token_n = torch.repeat_interleave(token_n.unsqueeze(0), b, dim=0) # b s c

            Fp_list_n = few_shot_samples['patch'].cuda(non_blocking=True) # m s c h w
            Fp_list_n = torch.repeat_interleave(Fp_list_n.unsqueeze(0), b, dim=0) # b m s c h w
            Fp_list_n = rearrange(Fp_list_n, 'b m s c h w -> b m (h w s) c')

            _, m, l, c = Fp_list.shape
            ...
            for i in range(len(token)):
                ...
                for j in range(len(Fp)):
                    ...
                    for k in range(len(tmp_x)):
                        ...
                        s, _ = torch.sort((0.5 * (1 - (tmp @ tmp_n.T))), dim=-1, descending=False)
                        am_fp.append(s)
            ...
            patch_ref_map = patch_ref_map[:, :, :256]

            # 取最小值
            patch_ref_map_res = patch_ref_map[:, :, 0]
            patch_ref_max = torch.amax(patch_ref_map_res, dim=-1)

        return self.prototype_global(), self.prototype_patch(), token, Fp_list, patch_ref_max # 2, 1024 | b c | b m l c
